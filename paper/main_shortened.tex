...
In-context learning (ICL) enables large language models to perform new tasks from demonstrations without parameter updates \citep{brown2020language,garg2022can}, an emergent capability \citep{wei2022emergent} central to mechanistic interpretability and activation steering \citep{turner2023activation}. Prior work uses linear probing to localize task representations \citep{todd2023function,hendel2023context}, finding high probe accuracy. However, \emph{decodability} does not imply \emph{causal relevance} \citep{ravfogel2020null,elazar2021amnesic}.

We investigate causal task representations through activation intervention on Llama-3.2-3B-Instruct. Our central finding: \textbf{single-position intervention fails completely}. Despite 100\% probing accuracy at demo positions and 83\% at query, replacing any single (layer, position) yields 0\% transfer across all 28 layers. Control experiments confirm these positions are not causally necessary---the model ignores single-position perturbations, suggesting distributed encoding.

The breakthrough comes from \textbf{multi-position intervention}. Simultaneously replacing activations at all demo token positions achieves 96\% transfer for format-compatible pairs (N=50, 95\% CI: [87\%, 99\%]). Both \texttt{all\_demo} (96\%) and \texttt{output\_only} (94\%) succeed, while \texttt{input\_only} yields 0\%---output tokens carry the primary signal. This works only at $\sim$30\% depth (layer 8/28) and only when tasks share compatible output formats. Findings replicate across four models (Llama-3B, Llama-1B, Qwen-1.5B, Gemma-2B) with optimal layer consistently at $\sim$30\% depth.
...
\begin{itemize}
    ...
    \item \textbf{Causal dissociation:} Query is necessary but not sufficient; demos are collectively sufficient but individually not necessary. Transfer scales sharply with output position count (1--3→0\%, 10→10\%, all→90\% mean) and source demos (1--3→0\%, 5→93\%).
...
We study ICL in the standard few-shot setting: given $k$ demonstration pairs $\{(x_i, y_i)\}_{i=1}^k$ and query $x_q$, the model infers the task from demonstrations alone. Our goal is identifying which (layer, position) pairs encode task identity \emph{causally}, distinguishing decodability from causal relevance.

\citet{olsson2022context} identified induction heads as key ICL circuits; activation patching \citep{vig2020investigating,meng2022locating} finds localized sites for factual recall. Our single-position failure suggests ICL task identity is mechanistically distinct from factual knowledge.
...
An ICL prompt consists of $k$ demonstration pairs followed by a query. We identify three position types: \textbf{demo input positions} (tokens in $x_i$), \textbf{demo output positions} (tokens in $y_i$), and \textbf{query positions}. This distinction proves critical---demo output positions carry the primary task signal. After intervention, outputs are classified as \textbf{transfer} (correct for source task), \textbf{preserve} (correct for target), or \textbf{malformed}.
...
We train nearest-centroid classifiers at each (layer, position) pair. We probe three positions: \texttt{last\_demo\_token}, \texttt{separator}, and \texttt{first\_query\_token}.

\subsection{Single-Position Intervention}

For source task $\mathcal{T}_s$ and target $\mathcal{T}_t$, we compute mean source activations and replace: $h_\ell^{(p)} \leftarrow \bar{h}_s$. We test all 28 layers. Zero and random ablation controls test whether positions are causally necessary.

\subsection{Multi-Position Intervention}

Multi-position intervention replaces activations at multiple positions simultaneously, testing whether task identity is distributed. Four scopes: \texttt{all\_demo}, \texttt{input\_only}, \texttt{output\_only}, and \texttt{last\_demo}. We test layers 8, 12, 14, and 16.
...
\paragraph{Models.} Primary experiments use \textbf{Llama-3.2-3B-Instruct} (28 layers); replication uses Llama-1B (16 layers), Qwen-1.5B \citep{bai2023qwen} (28 layers), Gemma-2B \citep{team2024gemma} (26 layers). We use 5-shot prompts and greedy decoding.

\paragraph{Metrics.} We report \textbf{transfer rate} $\tau$ (fraction producing source-task outputs) and \textbf{disruption rate} $\delta$ (accuracy drop under noise). Each experiment uses 10 test inputs per task pair across 56 combinations.
...
\paragraph{Output tokens carry task identity.} \texttt{output\_only} achieves 94\% while \texttt{input\_only} yields 0\%; \texttt{last\_demo} (single pair) also yields 0\%, confirming all demo outputs must be replaced.

\paragraph{Multi-model replication.} Transfer replicates across four models: Llama-3B (90\%), Llama-1B (60\%), Qwen-1.5B (90\%), Gemma-2B (40\%), with optimal layer consistently at $\sim$30\% depth.
...
Noise injection reveals a striking dissociation (Table~\ref{tab:causal}, Figure~\ref{fig:causal}). Query is \emph{necessary} (53--100\% disruption, layers 0--14) but not sufficient (0\% transfer). Individual demos show the opposite: 0\% disruption when noised, yet collectively sufficient (96\% transfer). This reveals distributed encoding: information spreads across demo outputs and aggregates to query.
...
\paragraph{What determines compatibility?} Surface similarity (r=$-$0.05) and activation similarity (r=0.31) fail to predict transfer, but \textbf{task cluster membership is predictive}: all 7 high-transfer pairs involve procedural tasks with matching output token counts. The asymmetry (uppercase→linear\_2x = 100\%, reverse = 0\%) follows from token-count compatibility.
...
Four lines of evidence support this: (1) \emph{Probe trajectories}: demo positions reach 100\% by layer 4; query peaks at 83\% at layer 12. (2) \emph{Attention}: query attends to demo outputs (0.38) over inputs (0.12). (3) \emph{Layer ablation}: layer 0 is critical; layer 16 preserves 72\%. (4) \emph{Scaling thresholds}: position count (1--3→0\%, 10→10\%, all→90\%) and source demos (1--3→0\%, 5→93\%) both show sharp thresholds.

\paragraph{The template hypothesis.}
What transfers is output templates---structural patterns like ``[WORD]'' or ``[WORD] [SEP] [WORD]''. Token-count is critical: repeat\_word→repeat\_n transfers at 100\% when N=2 but 0\% for N$\geq$3. Templates are abstract: sentiment variants transfer at 95--100\%, explaining why format changes block transfer while structural matches succeed.

\paragraph{Task complexity.}
Semantic tasks show near-zero cross-task transfer, but sentiment variants (same task, different labels) transfer at 95--100\%---the mechanism operates when output templates align.

\section{Related Work}
\label{sec:related}

\paragraph{Function vectors and task representations.}
\citet{todd2023function} extracted function vectors by averaging activations; \citet{hendel2023context} applied task vectors to new prompts. Our work explains why: averaging implicitly captures distributed encoding. This predicts single-demo function vectors should fail.

\paragraph{Distributed representations.}
Multi-position intervention aligns with distributed coding \citep{hinton1986distributed,elhage2022superposition}. Prior work noted format sensitivity \citep{lu2022fantastically,zhao2021calibrate}; our hypothesis extends this to internal activations.
...
\newpage
\section*{Extended Discussion: Model, Contributions, and Future Directions}

\subsection*{Target Model and Problem Context}

We target \textbf{Llama-3.2-3B-Instruct}, a decoder-only transformer with 28 layers and 3.21B parameters. The core problem is \textbf{in-context learning (ICL)}: adapting to new tasks from few-shot demonstrations without parameter updates \citep{brown2020language}. Understanding ICL mechanisms addresses fundamental questions about how neural networks represent learned functions.

The literature approaches ICL through multiple lenses: induction heads \citep{olsson2022context}, function classes \citep{garg2022can}, implicit gradient descent \citep{dai2022can,vonoswald2023transformers}, and function vectors \citep{todd2023function,hendel2023context}. However, prior work relies on probing---measuring information presence, not causal relevance \citep{belinkov2019analysis}. Our intervention experiments address this gap.

\subsection*{Contributions to Model Understanding}

\textbf{1. Causal structure of task representations.} We establish that ICL task identity is distributed across demonstration output tokens, not localized to any single position. The finding that 100\% probing accuracy yields 0\% single-position transfer demonstrates the probe-causation gap concretely.

\textbf{2. Processing stages in transformers.} The $\sim$30\% depth finding across four models suggests a universal processing stage: early layers encode templates, mid layers aggregate to query, late layers commit to output.

\textbf{3. Format over semantics.} The template hypothesis---that models encode output format patterns rather than abstract task identity---explains why identical operations with different formats show zero transfer.

\textbf{4. Necessity vs. sufficiency dissociation.} Query position is necessary (53--100\% disruption) but not sufficient (0\% transfer); demo positions are collectively sufficient but individually not necessary. This maps the causal architecture of ICL information flow.

\subsection*{Influence on Future Work}

\textbf{Mechanistic interpretability:} Single-position patching fails for ICL, unlike factual editing \citep{meng2022locating}. Future work should validate localization with causal interventions, not just probing.

\textbf{Activation steering:} Coordinated multi-position intervention at $\sim$30\% depth is required. Format compatibility predicts amenable task pairs.

\textbf{Robustness:} Distributed encoding explains ICL's robustness to perturbations.

\textbf{Scaling:} Testing whether distributed templates hold at 7B--70B scales provides testable predictions.
