...
Function vectors \citep{todd2023function,hendel2023context} succeed because averaging captures distributed encoding. Our multi-position intervention aligns with distributed coding \citep{hinton1984distributed}. Prior work noted format sensitivity \citep{lu2022fantastically,min2022rethinking}; our template hypothesis extends this to activations.
...
The fault tolerance we observe (0\% disruption when any single demo position is noised) is a hallmark of distributed coding \citep{hinton1984distributed}. The position count ablation (Table~\ref{tab:position_count}) confirms there is no critical subset---transfer shows a sharp threshold requiring nearly all positions (Figure~\ref{fig:scaling}).
